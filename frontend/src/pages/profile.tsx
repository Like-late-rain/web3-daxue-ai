...
export default function Profile() {
  ...
  const { data: courseIds } = useStudentCourses(address) as { data: bigint[] | undefined };
              ...
              <div className="flex-1 text-center md:text-left">
                <div className="flex flex-col md:flex-row md:items-center md:space-x-3 mb-3">
                  {isEditingNickname ? (
                    <input
                      ...
                      className="px-4 py-2 bg-dark-card/50 border border-cyber-cyan/30 rounded-lg text-white focus:outline-none focus:border-cyber-cyan focus:ring-2 focus:ring-cyber-cyan/30 transition-all"
                      placeholder="输入昵称"
                    />
                  ) : (
                ...
                {ensName && (
                  <div className="inline-flex items-center px-3 py-1 glass rounded-full border border-cyber-cyan/30">
                    <span className="text-cyber-cyan text-sm font-medium">{ensName}</span>
                  </div>
                )}
              </div>
            </div>
